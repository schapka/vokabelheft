 ...
 *
 *   pnpm new-lesson --title "Vokabelliste 1.3" [--language en-GB] [--reference 1-3] [--date 2026-09-20]
 ...
 * next free number in that school year, generates the id, and writes
 * data/lessons/<school-year>-<nn>-<language>-<reference>.json with empty
...
if (!values.title) {
  console.error('usage: pnpm new-lesson --title "Vokabelliste 1.3" [--language en-GB] [--reference 1-3] [--date YYYY-MM-DD]')
  process.exit(1)
}
...
const existingNumbers = readdirSync(LESSONS_DIR)
  ...
  .filter(parts => parts !== null && parts.schoolYear === schoolYear)
  .map(parts => parts!.number)
const number = Math.max(0, ...existingNumbers) + 1

const fileName = formatLessonFileName({ schoolYear, number, language, reference })
